...
==================================================================
== REWRITTEN TO .NET AND BRUTE FORCED ON 20 THREADS, 12 minutes ==
==================================================================
 * 
 */
...
export async function day5b(dataPath?: string) {
  ...
  for (let i = 0; i < seeds.length; i = i + 2) {
    ...
    for (let seed = startSeed; seed < lastSeed; seed++) {
      let location = relations.resolveLocation(seed);
      if (location < min) {
        min = location;
...
class CleverMap {
  ...
  public getRelation(source: number): number {
    ...
    for (const mapEntry of this.map) {
      if (
        mapEntry.sourceStart <= source &&
        mapEntry.sourceStart + mapEntry.length >= source
      ) {
        let resolution =
          mapEntry.destionationStart + source - mapEntry.sourceStart;
